document.querySelectorAll(".slider_subcategory").forEach(wrapper => {
  let carousel = wrapper.querySelector(".list_subcategory");
  let btns = wrapper.querySelectorAll(".btn_slider_subcategory");
  let carouselChildren = [...carousel.children];

  let timeAutoPlay = 500;

  // getting card width
  let cardWidth = carousel.querySelector(".subcategory").offsetWidth;
  ...
  // getting number of cards that can fit in carousel at once
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      carousel.scrollLeft += btn.className == "btn_slider_subcategory fas fa-angle-left" ? -cardWidth : cardWidth;
    });
  });
  ...
  let infiniteScroll = () => {
    ...
    // else if carousel is at end, scroll to beginning
